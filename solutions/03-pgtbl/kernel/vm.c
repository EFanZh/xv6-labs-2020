...
static void vmmap(pagetable_t pagetable, uint64 va, uint64 pa, uint64 sz, int perm) {
 ...
 */
void kvminit() {
    kernel_pagetable = create_kernel_pagetable();
}
...
void vmprint(pagetable_t page_table) {
    const unsigned int num_ptes = 512;

    printf("page table %p\n", page_table);

    for (unsigned int i = 0; i != num_ptes; ++i) {
        const pte_t pte_1 = page_table[i];
        const pagetable_t table_1 = (pagetable_t)PTE2PA(pte_1);

        if ((pte_1 & PTE_V) != 0) {
            printf("..%d: pte %p pa %p\n", (int)i, (const void *)pte_1, table_1);

            for (unsigned int j = 0; j != num_ptes; ++j) {
                const pte_t pte_2 = table_1[j];
                const pagetable_t table_2 = (pagetable_t)PTE2PA(pte_2);

                if ((pte_2 & PTE_V) != 0) {
                    printf(".. ..%d: pte %p pa %p\n", (int)j, (const void *)pte_2, table_2);

                    for (unsigned int k = 0; k != num_ptes; ++k) {
                        const pte_t pte_3 = table_2[k];
                        const pagetable_t table_3 = (pagetable_t)PTE2PA(pte_3);
